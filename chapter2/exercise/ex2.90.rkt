...
#|
;这里自己实现的不对，但网上答案看不明白
(define (install-sparse-terms-poly)
  (define (tag x)(attach-tag 'sparse-polynomial))
  (define (empty-termlist? terms)(null? terms))
  (define (the-empty-termlist) null)
  (define (poly? x)
    (eq? 'sparse-polynomial (type-tag x)))
  ...
  (put '=zero? 'sparse-polynomial
       zero-poly?)
  (put 'get-negtive '(sparse-polynomial)
       get-negtive-poly)
  
  'done)


(define (install-dense-terms-poly)
  (define (tag x)(attach-tag 'dense-polynomial))
  (define (empty-termlist? terms)(null? terms))
  (define (the-empty-termlist) null)
  (define (poly? x)
    (eq? 'sparse-polynomial (type-tag x)))
  ...
  (put '=zero? 'dense-polynomial
       zero-poly?)
  (put 'get-negtive '(dense-polynomial)
       get-negtive-poly)
...
(define (install-dense-term)
  ...
  (define (adjoin-term-dense term term-list)
    (cond ((=zero? term) term-list) 
          ...
          (else (adjoin-term term (cons 0 term-list)))))
  ;转换
  (define (first-term-sparse terms)
    ())
  ;转换
  (define (adjoin-term-dense term term-list)
    ())
  (define (adjoin-term-sparse term term-list)
    (if(=zero? (coeff term))
       (term-list)
       (cons term term-list)))
  (put 'first-term-sparse '(dense-term)
       first-term-sparse)
  (put 'adjoin-term-sparse '(dense-term)
       adjoin-term-sparse)
  (put 'first-term-dense '(dense-term)
       first-term-dense)
  (put 'adjoin-term-dense '(dense-term)
       adjoin-term-dense)
  'done)

(define (install-sparse-term)
  ;多项式系数表相关——构造器选择器——提供对外接口
  (define (first-term-sparse terms)(car terms))
  (define (adjoin-term-sparse term term-list)
    (if(=zero? (coeff term))
       (term-list)
       (cons term term-list)))
  (define (first-term-dense term)
    (make-term (- (length term-list) 1) (car term-list)))
  (define (adjoin-term-dense terms)
    (cond ((=zero? term) term-list) 
          ((equ? (order term) (length term-list)) (cons (coeff term) term-list)) 
          (else (adjoin-term term (cons 0 term-list)))))
  (put 'first-term-sparse '(sparse-term)
       first-term-sparse)
  (put 'adjoin-term-sparse '(sparse-term)
       adjoin-term-sparse)
  (put 'first-term-dense '(sparse-term)
       first-term-dense)
  (put 'adjoin-term-dense '(sparse-term)
       adjoin-term-dense)
  'done)
